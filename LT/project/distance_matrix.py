...
output_pop = 'LT/project/txt_files/10cities_pop.txt'
output_times = 'LT/project/txt_files/10cities_time.txt'
...
def main():
    print(msg)
    ten_cities(selected_cities, additional_entries)



##

def ten_cities(L, additional_entries):
    '''
    gather into files the useful datas for the specific problem of Camille.
    L correspond to the indexes of of the corresponding cities: population & time.
    Function created on 8.12.
    '''


    useful_entries = [raw_data[i] for i in L]
    if verbose: 
...
def custom_save(file,R):
    """
    custome save of datas of R into file (used for CPLEX resolution)
    """
    f = open(file, 'w')
    line = '['
    for entry in R:
